...
`alloc_null_no_memerror` findings carry both `line` (the allocation) and `guard_line` (the `if (x == NULL)`). `pylong_sentinel_no_errcheck` carries `line` (the `== -1` guard) and `assign_line` (the conversion). `int_status_never_tested` carries `line` (the assignment) and `use_line` (the `return var;` where the status finally matters).

**Why `int_status_never_tested` is a separate rule from `unchecked_return`.** The "value flows straight back to the caller" suppression is correct for a *pointer* — the callee's exception propagates untouched and nothing in between dereferences the NULL. For an **int status** it is wrong: `return res` at the *end* of the function stops nothing in between. `Objects/typeobject.c:1966` assigns `add_all_subclasses(...)` to `res`, never tests it, then falls into `update_all_slots()` with a live exception and skips its own rollback — SIGABRT on debug (`assert(!PyErr_Occurred())` in `update_one_slot`), and on release `__bases__` is committed while `MemoryError` is raised and the class ends up in **neither** parent's `__subclasses__()`. Reproduced as CPY-0070; bpo-38554 / PR gh-16879 proposed this exact diff in 2019 and it was closed unmerged. The old rule could not reach it at all: `_ASSIGN_CALL_RE` is built from the NULL-returning API table and `add_all_subclasses` is a same-file `static int` helper, so there was never a candidate to suppress.

The callee set is **discovered** per file (any `static int` helper whose body mentions `-1`) plus a table of int-status C-API calls. The load-bearing gate is that the region between the assignment and the read must be *fallible or committing* — a `goto`, a `return NULL/-1`, a `PyErr_*` call, or another tested call. Without it the count is **160** tree-wide; with it, 2. One of those 2 is the documented FP class (required symmetric cleanup, `Modules/_pickle.c` `save_frozenset:3796`) — check `data/cpython_non_bugs.md` and dismiss it first.
