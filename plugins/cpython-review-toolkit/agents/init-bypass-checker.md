...
- `findings[].sink_kind`: `incref` / `call` / `deref` / `addr_deref` (the sink is written in this function), `one_hop_param_deref` (**the sink is one hop away** — see below), or `vararg_sentinel` (a truncation, not a crash).
- `findings[].callee` / `callee_param_index` / `callee_deref_line`: for `one_hop_param_deref`, the same-file `static` callee the nullable field is handed to, which argument slot it lands in, and **the line inside the callee where it is dereferenced**. Cite that line as the crash site, and the finding's own `line` as where the bad value is passed.
- `findings[].dominated_by` / `dominates`: within one function, a sink that is downstream of another on the *same path* can never be reached once the earlier one crashes. `scan_init_bypass` used to report CPY-0007 at `super_descr_get:12806` (`Py_NewRef(su->type)`), which is **dead code** — control dies at `:12797` inside `supercheck`, whose first parameter is dereferenced at `type->tp_name` with no guard. The catalog entry has the right lines, so the scanner was right by luck. Both sinks are still reported (suppressing one would hide a real sink if the path analysis is ever wrong), but **if a finding carries `dominated_by`, cite the dominating line instead**. Path exclusivity is computed from the AST, not from brace counting, because CPython writes the braceless `if (c) return f(x); else { ... }` where text-order counting cannot tell the two arms apart.

**`vararg_null_truncation` is a different bug, not a crash.** The `*ObjArgs` family terminates its argument list at the first NULL, so a nullable field in a **non-final** slot does not segfault and does not raise — the call silently becomes a shorter call than the one written. `super_descr_get:12793` turns a 2-argument `super()` construction into a **0**-argument one and drops `obj` (CPY-0080). Classify as **CONSIDER** (wrong behaviour, silent) unless you can show the truncated call is itself a crash. `arguments_dropped` lists what is lost. Argument 0 is deliberately excluded — a NULL callable raises, and that is the ordinary `call` sink.
...
This is not hypothetical, and it has now failed twice. Before this rule existed the scanner returned `nullable_fields: 0` on the pre-gh-144330 `Objects/funcobject.c` — a file that provably contained the bug — and 0 across all of `Objects/`, while a hand-read of the same scope found a live SIGSEGV in `bytearray`. The second time was the `PyType_Spec` **whole-file kill switch**: the scanner disabled the `new_bypass` signal for an entire file as soon as the token `Py_tp_new` appeared anywhere in it, even though `{Py_tp_new, PyType_GenericNew}` is the canonical *bypassable* wiring and `Py_TPFLAGS_DISALLOW_INSTANTIATION` on one type says nothing about its siblings. That silenced **21 of the 58 slot tables carrying a `Py_tp_init` tree-wide (36%)** — including `_asynciomodule.c` Future *and* Task, all seven `_ctypes` metatypes, `_decimal` `signaldict`, `Python-ast.c` `AST_type_slots`, and `selectmodule.c` `kqueue_event` — and hid `_asyncio.Task.__new__(_asyncio.Task).get_context()`, an exit-139 SIGSEGV on all four build variants. All three slot-table forms now pair `tp_init` with the `tp_new` of the *same* table.

Reference points on current main: `Objects/` = 38 nullable fields / 3 findings, `Modules/` = 123 / 26, `Python/` = **0 / 0** (genuinely: every deletable member there is `Py_READONLY` and the one `tp_init` has a real `tp_new`, but treat it as unexercised, not as verified-clean). One known-class false positive in that `Modules/` count: `_pickle.c:1103` `_Pickler_Write` reading `output_buffer` — every caller passes through `_pickle_Pickler_dump_impl:4881`, whose `issue3664` guard rejects an uninitialised Pickler. That is the documented interprocedural limitation, not a new defect.
