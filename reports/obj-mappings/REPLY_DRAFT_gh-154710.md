...
So a callback that does exactly what it's told — returns `-1` on failure — causes CPython itself to execute arbitrary Python through `sys.unraisablehook`, at precisely the moment the contract says code must not run. A conforming callback can't avoid it. That's why I don't think this is only a dict-ordering bug: the contract as written can't be satisfied by anyone.
