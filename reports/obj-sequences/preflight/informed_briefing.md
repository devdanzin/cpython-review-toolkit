...
From the `cpython-review-findings` catalog (171 recorded). These are context, not fresh discoveries: confirm each still reproduces in one line, then spend your effort on **un-found siblings** of the same shape (via its guarded twin).
...
| CPY-0162 | null-deref | `Modules/_io/bufferedio.c:591 (_io__Buffered_close_impl); Modules/_io/bufferedio.c:788 (_buffered_raw_tell); Modules/_io/bufferedio.c:818 (_buffered_raw_seek); Modules/_io/bufferedio.c:1389 (_io__Buffered_seek_impl); (+7 more)` | gh-143008's buffer_access_safe hardening was applied to textio.c only: bufferedio.c dispatches on a detached (NULL) self->raw at six sites -> SIGSEGV from pure Python on released CPython 3.14 | reproduced |
| CPY-0163 | memory | `Modules/_io/bufferedio.c:847 (_buffered_init); Modules/_io/bufferedio.c:854 (_buffered_init); Modules/_io/bufferedio.c:1604 (_io_BufferedReader___init___impl)` | _buffered_init frees self->buffer and self->lock while a frame is using them: BufferedReader/Writer/Random.__init__ takes neither ENTER_BUFFERED nor a critical section -> heap-use-after-free WRITE and PyMutex_Unlock abort from pure Python | reproduced |
| CPY-0164 | memory | `Modules/_io/bytesio.c:748 (_io_BytesIO_truncate_impl); Modules/_io/bytesio.c:761 (_io_BytesIO_truncate_impl); Modules/_io/bytesio.c:110 (resize_unshared_buffer_lock_held)` | BytesIO.truncate() runs CHECK_EXPORTS before PyLong_AsLong dispatches a user __index__: a callback taking getbuffer() makes the following resize realloc a buffer a live memoryview points into -> heap-use-after-free | reproduced |
| CPY-0165 | memory | `Modules/_io/bytesio.c:1282 (bytesiobuf_getbuffer_lock_held); Modules/_io/bytesio.c:1296 (bytesiobuf_getbuffer)` | bytesiobuf_getbuffer has no closed check and the intermediate is reachable as memoryview.obj -> SIGSEGV on all builds | reproduced |
| CPY-0166 | correctness | `Modules/_io/bytesio.c:1288 (bytesiobuf_getbuffer_lock_held); Modules/_io/bytesio.c:1291 (bytesiobuf_getbuffer_lock_held)` | bytesiobuf_getbuffer casts away PyBuffer_FillInfo's result on a wrong premise: returns success with the Py_buffer unwritten, an exception set, and exports permanently leaked | reproduced |
| CPY-0167 | tsan | `Modules/_io/fileio.c:501 (_io_FileIO___init___impl)` | FileIO.__init__ dereferences a NULL self->stat_atopen at fileio.c:501 -- the window is an explicit Py_BEGIN_ALLOW_THREADS, so it crashes on GIL builds too | reproduced |
| CPY-0168 | crash | `Modules/_io/textio.c:1365 (textiowrapper_change_encoding); Modules/_io/textio.c:1501 (_io_TextIOWrapper_reconfigure_impl)` | TextIOWrapper.reconfigure() is the only method in textio.c with neither CHECK_ATTACHED nor CHECK_INITIALIZED: Py_INCREF on a NULL self->encoding -> SIGSEGV on released CPython 3.14 | reproduced |
| CPY-0169 | crash | `Modules/_io/textio.c:1783 (_io_TextIOWrapper_write_impl); Modules/_io/textio.c:2775 (_io_TextIOWrapper_seek_impl); Modules/_io/textio.c:2873 (_io_TextIOWrapper_tell_impl); Modules/_io/textio.c:2937 (_io_TextIOWrapper_tell_impl)` | gh-143008's buffer_access_safe is field-specific: self->encoder, self->decoder and self->snapshot in the same file are read after user Python with no re-check -> SIGSEGV | reproduced |
| CPY-0170 | memory | `Modules/_io/textio.c:350 (_PyIncrementalNewlineDecoder_decode); Modules/_io/textio.c:511 (_PyIncrementalNewlineDecoder_decode); Modules/_io/textio.c:1216 (_io_TextIOWrapper___init___impl)` | _PyIncrementalNewlineDecoder_decode reads and writes its own self after the nested user codec call freed it: heap-use-after-free invisible on non-ASan builds | reproduced |
| CPY-0171 | tsan | `Modules/_io/textio.c:639 (incrementalnewlinedecoder_newlines_get); Modules/_io/textio.c:644 (incrementalnewlinedecoder_newlines_get)` | gh-144777's critical-section fix guarded decode/getstate/setstate/reset and missed the hand-written newlines getter: TSan race at textio.c:644 against all four guarded writers | reproduced |
| CPY-0172 | memory | `Modules/_io/bufferedio.c:2018 (_bufferedwriter_raw_write); Modules/_io/bufferedio.c:2232 (_io_BufferedRWPair_write_impl)` | A nested Buffered used as a borrowed receiver is freed by user code, then LEAVE_BUFFERED calls PyThread_release_lock on the freed object -> SIGSEGV on all builds | reproduced |
| CPY-0173 | correctness | `Modules/_io/bufferedio.c:495 (_io__Buffered__dealloc_warn_impl); Modules/_io/bufferedio.c:870 (_buffered_init); Modules/_io/bufferedio.c:1490 (_io__Buffered_truncate_impl)` | _io__Buffered__dealloc_warn_impl clears whatever the user's ResourceWarning handler raised, swallowing KeyboardInterrupt and MemoryError, where its fileio twin saves, narrows and reports | reproduced |
| CPY-0174 | tsan | `Modules/_io/bytesio.c:1311 (bytesiobuf_releasebuffer); Modules/_io/bytesio.c:1305 (bytesiobuf_getbuffer)` | bytesiobuf_releasebuffer takes no critical section while its getbuffer twin ten lines above does: the relaxed exports load never acquires against the seq-cst decrement | reproduced |
| CPY-0175 | memory | `Modules/_io/bufferedio.c:2286 (_io_BufferedRWPair___init___impl); Modules/_io/bufferedio.c:2292 (_io_BufferedRWPair___init___impl)` | BufferedRWPair.__init__ called twice permanently leaks the previous reader and writer, while BufferedReader.__init__ and TextIOWrapper.__init__ in the same package release theirs correctly | reproduced |
| CPY-0176 | memory | `Modules/_io/textio.c:977 (_textiowrapper_set_decoder); Modules/_io/textio.c:1034 (_textiowrapper_set_encoder)` | _textiowrapper_set_decoder stores into self->decoder without releasing a value a re-entrant codec factory already stored there: the intermediate decoder leaks | reproduced |
| CPY-0177 | correctness | `Modules/_io/bytesio.c:1081 (bytesio_dealloc)` | bytesio_dealloc mutates exception state with no save/restore bracket and emits no unraisable report where _pyio reports the BufferError | reproduced |
| CPY-0178 | correctness | `Modules/_io/bytesio.c:748 (_io_BytesIO_truncate_impl); Modules/_io/stringio.c:472 (_io_StringIO_truncate_impl)` | BytesIO.truncate and StringIO.truncate convert their size argument with PyLong_AsLong where the field is Py_ssize_t: correct on LP64, rejects valid sizes on LLP64 | reproduced |
| CPY-0179 | correctness | `Modules/_io/fileio.c:296 (_io_FileIO___init___impl); Modules/_io/fileio.c:303 (_io_FileIO___init___impl); Modules/_io/winconsoleio.c:69 (_PyIO_get_console_type)` | FileIO.__init__ clears whatever the caller's __index__ raised while deciding fd-or-path: KeyboardInterrupt, SystemExit and MemoryError all become 'expected str, bytes or os.PathLike object' | reproduced |
